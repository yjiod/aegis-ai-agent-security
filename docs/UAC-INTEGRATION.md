...
### UAC 门户环境推导
UAT 环境 = 二级域名末尾加 `uat`：
- 生产门户: `https://<UAC_DOMAIN>/#/c-login`
- UAT 门户:  `https://<UAC_UAT_DOMAIN>:10201/#/c-login` （**带端口 10201**，443 返回 503）
...
| 环境 | 网关 |
|------|------|
| DEV | https://<PAAS_DOMAIN> |
| TEST | https://test-paas.<CORP_DOMAIN> |
| UAT | https://uat-intra-paas.<CORP_DOMAIN> |
| PROD-深圳 | https://sz-intra-paas.<CORP_DOMAIN> |
| PROD-香港 | https://<PAAS_HK_DOMAIN> |
| PROD-法兰克福 | https://<PAAS_FRA_DOMAIN> |

### 配置（服务器 `/etc/aegis/console.env`）
```bash
AEGIS_AUTH_PROVIDER=uac
AEGIS_UAC_GATEWAY=https://sz-intra-paas.<CORP_DOMAIN>   # 按部署地域选择
AEGIS_UAC_APP_ID=<开放平台 App ID>
AEGIS_UAC_APP_SECRET=<开放平台 App Secret>
AEGIS_UAC_PORTAL=https://<UAC_DOMAIN>/#/c-login   # 可选, 默认此值
...
2. 点击 → 重定向 UAC 门户:
   https://<UAC_DOMAIN>/#/c-login?appId=<APP_ID>&redirect=<控制台回调>
3. 用户在 UAC 完成登录
...
- 证书：Let's Encrypt，`/etc/nginx/ssl/aegis.example.com.{crt,key}`
