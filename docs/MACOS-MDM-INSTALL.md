...
管理员交互入口 `aegis-install-macos-oneclick.sh` 由发布构建从本脚本生成，逐字节一致，并纳入企业包及摘要清单。两个入口均支持公开工件参数 `-PkgSha256`、`-TeamId`、`-PkgUrl` 或 `-PkgPath`；参数覆盖对应环境变量，重复、缺值、未知参数和来源冲突拒绝。交互入口不再自行提权、提前停旧服务、下载未验证包或写服务器覆盖文件；旧 `-Server` 参数拒绝。控制台部署不得再注入改写 Mac 脚本内容。见 [管理员安装说明](../public/downloads/MACOS-INSTALL.md)。

## 安装顺序与拒绝条件

1. 检查管理员身份、macOS、输入及旧版服务状态。发现旧系统服务、常见用户 LaunchAgent 或待核实的清理标记时，返回迁移/核实状态，不停止旧服务、不下载或替换文件。当前不承诺覆盖所有自定义历史服务位置。
...
5. 再次校验整包摘要，随后调用系统 Installer。Installer 成功仅代表安装步骤成功，仍需客户端诊断、入网及 Collector 接收证据。
...
标准输出为 `aegis.mdm-install-result/v1`，只包含固定状态、是否尝试安装、Installer 是否成功、健康是否验证及合法格式的工件摘要。原始下载、签名、公证和安装器输出不上传为 MDM 结果。
...
| `legacy_service_migration_required` | 先完成有证据的旧服务迁移；当前入口不自动迁移或清理 |
...
健康检查使用 `mdm-macos-compliance.sh` 调用客户端内嵌诊断；受保护上报配置使用 [原生配置入口](MACOS-CONFIGURATION.md)。历史 `rollback-aegis-macos.sh` 管理的是 Python 三文件安装布局，不能用于本原生包。原生安装回滚、旧用户服务自动迁移、真实签名公证及完整生命周期验收仍为发布前待办，见 [R7 契约](MACOS-RUNTIME-CONTRACT.md)。
