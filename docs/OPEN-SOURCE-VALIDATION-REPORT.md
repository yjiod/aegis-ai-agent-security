...
> 验证宿主：192.0.2.132（x86_64 / 30GB / 与 aegis.example.com 同 mesh-overlay）
...
- 现网商用（<EDR_VENDOR_CN> EDR / <MDM_VENDOR_CN>桌管 / Intune）与开源候选**地位对等**：换厂商=换适配器+改配置。
...
1. 服务器备 pkg：wazuh `curl -sL -o /tmp/wazuh-agent.pkg https://packages.wazuh.com/4.x/macos/wazuh-agent-<ver>-<rev>.pkg`；fleet `fleetctl package --type orbit --macos --fleet-url http://192.0.2.132:18080 --enroll-secret <SECRET> --insecure`。
2. scp pkg 到 mac，mac 上：`./mac-enroll.sh wazuh /tmp/wazuh-agent.pkg 192.0.2.132` / `./mac-enroll.sh fleet /tmp/orbit.pkg`。
