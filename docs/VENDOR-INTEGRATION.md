...
<EDR_VENDOR_CN> EDR 6.x 与 5.x 的主要差异：
...
Authorization: Bearer {<EDR_VENDOR_UPPER>_EDR_TOKEN}
...
1. 向<EDR_VENDOR_CN> EDR 管理员申请服务账号 + API Token
2. 确认现网 EDR 版本号（不同版本 API 路径可能不同）
3. 确认 EDR 管理面 HTTPS 地址，加入 `allowed_hosts`
4. 设置环境变量 `<EDR_VENDOR_UPPER>_EDR_TOKEN=<实际token>`
5. 修改 `aegis-adapters.json` 中 `<edr_vendor>.enabled: true`
6. 先用 `--dry-run` 验证配置，再正式启用

## <MDM_VENDOR_CN>桌管接口契约

### 请求

```
POST https://{<mdm_vendor>-host}/api/aegis/posture
Authorization: Bearer {<MDM_VENDOR_UPPER>_TOKEN}
Content-Type: application/json
...
1. 向<MDM_VENDOR_CN>桌管管理员申请 API 服务账号
2. 确认<MDM_VENDOR_CN> <MDM_VIEW> 版本及 API 端点路径
3. 确认 HTTPS 地址，加入 `allowed_hosts`
4. 设置环境变量 `<MDM_VENDOR_UPPER>_TOKEN=<实际token>`
5. 修改 `aegis-adapters.json` 中 `<mdm_vendor>.enabled: true`
...
#    <edr_vendor>.url: "http://127.0.0.1:9443/api/aegis/events"
#    <mdm_vendor>.url: "http://127.0.0.1:9443/api/aegis/posture"
...
- [ ] <EDR_VENDOR_CN> EDR 正式 API 路径、鉴权方式已确认
- [ ] <MDM_VENDOR_CN>桌管正式 API 端点、服务账号已申请
