...
### 阶段 3：30k 容量优化
- 上报路径的**请求速率**：30k × 24 次/天 = 8.3 req/s 均值、峰值 ~50 req/s——单 PG 承受该速率没有问题。
  > **更正（原表述"单 PG 轻松承受"已被同文档 §6 推翻，此处改为限定口径）**：该结论**只对请求速率成立，对存储体量不成立**。30k 的瓶颈不是 req/s，而是 `reports` 表整包 body 落盘的**累计体量**与随之而来的写放大（详见 §6 与容量评估报告）。原表述省略了这个限定，会让读者误判"阶段 3 只是调优、可以最后做"。
...
- **上报体量（原阶段 3 缺失，据 §6 补入）**：差分/心跳上报（无变化只发 report_hash + scanned_at）+ 冷热分层 + 保留期下调。这一项**不是调优，是 30k 的前置条件**，实施顺序见 §7 第 3 步；
...
- 压测：3 万模拟终端 24h 连续上报，零丢失（collector 幂等去重保证）。
  > **当前不可验证**：仓库内**没有**合成舰队压测器，也没有 SLO 看板，因此本条与 §1/§6 的容量结论目前都只是推算。压测器应作为本条验收项的前置交付物（§7 已列入），否则"3 万终端 24h 零丢失"无法被证明。
...
### 6.1 两份文档的定位与权威关系（避免再次出现结论冲突）

| 文档 | 定位 | 权威范围 | 是否在仓 |
|---|---|---|---|
| 本文档（`HA-ARCHITECTURE.md`） | **架构决策记录（ADR）** | "做什么、按什么顺序做"——拓扑、分阶段实施、验收标准、实施顺序 | 是 |
| 30k 容量评估报告 | **容量评估证据** | "为什么是这个顺序、阈值取多少"——瓶颈测算、放量节奏、SLO 数值 | 否（内部归档） |

**冲突裁决规则**：两者结论不一致时，**以带磁盘或数据证据的一方为准**，并把该差异与裁定写进本文档（而不是各自表述）。本节即为按此规则处理的第一例——阶段 3 原表述"单 PG 轻松承受"只考虑了请求速率、未考虑存储体量，已在 §3 就地更正并限定口径。

**为什么 ADR 在仓、证据报告不在仓是可接受的**：ADR 的结论必须可被贡献者与审计方复核，因此必须在仓；容量测算的原始输入含生产环境细节，脱敏后入仓会损失其证据价值（详见 §6.2）。但**在仓文档不得引用读者拿不到的材料作为唯一依据**——因此本文引用 30k 报告时，一律同时把关键数值（2,160 万行 / 约 700 GB / 8.3 req/s 均值）直接写进正文，使结论可独立复核。

### 6.2 30k 容量评估报告是否入仓（脱敏清单）

结论：**建议脱敏后入仓**为 `docs/SCALE-30K-ASSESSMENT.md`。理由：放量决策会反复引用其 SLO 阈值与阶段划分，不在仓就等于每次都要向维护者索取，且 `SCALE-P1-1-FINDINGS-AGGREGATE.md` 已经依赖它。

实测脱敏面（对 284 行原文逐项扫描，只列类别与计数）：

| 类别 | 命中 | 处置 |
|---|---|---|
| 生产控制台域名 | **1 处**（第 174 行的部署流程描述） | 替换为占位域 `aegis.example.com` 或"生产控制台" |
| 开发主机标识 | **2 处**（第 174、248 行，均以"开发主机（本机 + X）"形式出现） | 替换为"开发主机"泛称，删除具体标识 |
| 仓库主名 / 组织名 | 1 处（同一行的 `tx.<主名>.com`） | 随域名一并占位化 |
| RFC1918 内网 IP | 0 | — |
| 硬编码令牌 / 密钥 | 0 | — |
| 本机绝对路径（`/Users/...`） | 0 | — |
| `.local` mDNS 主机名 | 0 | — |
| 设备序列号 / 工号 | 0 | — |
| 容量数值（GB / MB / vCPU / 内存，31 处） | 31 | **保留**。这是报告的结论本体，且属通用规格描述而非可定位到具体主体的标识；`privacy-scan.sh` 的三层规则也不覆盖它 |

**脱敏后必须跑的验证**（不能只看输出末行）：
1. `bash scripts/privacy-scan.sh`，**判退出码 = 0**。
2. 注意该脚本**层 2（本地 blocklist，明文敏感字面量）是 gitignored、只在维护者本机存在**，CI 只跑层 1（通用形状）+ 层 3（HMAC 指纹）。因此脱敏必须在**有 blocklist 的本机**跑一次，否则真实主机名/厂商名这类字面量能过 CI 却违反隐私铁律。本机 blocklist 实测存在（`scripts/privacy-blocklist.local`，权限 600）。
3. 新文件必须先 `git add` 再扫（脚本只扫 git 跟踪内容）。

**待用户裁定**：入仓与否最终属对外披露决策（该报告含内部放量节奏与服务器规格推断），本文只给出脱敏可行性与清单，不代替裁定。

## 7. 建议的实施顺序与文档整改

**实施顺序**（按"先解除硬约束、再去单点"排列，与 §3 的阶段编号不完全一致）：

1. **控制台进程内状态外移**（§3 阶段 1 第 2 步的前置）：后台循环移出 middleware，改独立 worker / systemd timer。不做这一步，多实例会重复执行自动纠偏与工单同步。
   > **选主方案更正**：本节原写"或 PG advisory lock 选主"，该选项**不成立**，已删除。依据：`lib/pg-store.ts` 的 `withClient` 每次操作新建 `Client` 并在 `finally` 中 `client.end()`（注释明写 "no pool, no reuse, no leak"），而 `pg_advisory_lock` 是**会话级**锁，连接一关闭锁即释放，无法跨越一个 5 分钟的 sweep 周期持有；要在锁内跑完就必须单开长连接，与该短连接模型直接冲突。若过渡期仍需在进程内选主，应改用**租约表**（`task_runs` 行 + `lease_expires_at`，短连接即可完成 CAS 抢占与续租），而非 advisory lock。
   > 另一条比多实例更早存在的缺陷：循环由 `middleware.ts` 在**请求路径**上启动，因此无请求时不会启动、isolate 被回收时静默消失且启动标志复位。移出请求路径同时修掉这一点。
