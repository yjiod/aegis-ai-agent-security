...
Simulates <EDR_VENDOR> EDR and <MDM_VENDOR> desktop management API endpoints.
...
Endpoints:
    POST /api/aegis/events     — <EDR_VENDOR> EDR event ingestion (returns 200 + JSON ack)
    POST /api/aegis/posture    — <MDM_VENDOR> compliance posture update (returns 200 + JSON ack)
...
<EDR_VENDOR_UPPER>_TOKEN = "mock-<edr_vendor>-edr-token-for-local-dev-only-32chars"
<MDM_VENDOR_UPPER>_TOKEN = "mock-<mdm_vendor>-token-for-local-dev-only-32chars-min"
...
class MockVendorHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/aegis/events":
            # <EDR_VENDOR> EDR event ingestion
            if not self._check_auth(<EDR_VENDOR_UPPER>_TOKEN):
                return
            ...
            record = {
                "vendor": "<edr_vendor>_edr",
                "event_id": event_id,
                "received_at": int(time.time()),
                "payload": payload,
            }
            received_events.append(record)
            print(f"  [<EDR_VENDOR> EDR] {payload.get('severity','?').upper()} | "
            ...
            # <MDM_VENDOR> compliance posture
            if not self._check_auth(<MDM_VENDOR_UPPER>_TOKEN):
                return
            payload = self._read_body()
            if payload is None:
                return
            ...
            print(f"  [<MDM_VENDOR_CN>桌管] {status} | "
                  f"{payload.get('device_id','?')} | "
            ...
            self._respond(200, {
                ...
                "message": "Mock <MDM_VENDOR>: posture recorded, no real enforcement applied",
            })
...
<EDR_VENDOR_UPPER>_EDR_TOKEN={<EDR_VENDOR_UPPER>_TOKEN}
<MDM_VENDOR_UPPER>_TOKEN={<MDM_VENDOR_UPPER>_TOKEN}
...
╠══════════════════════════════════════════════════════════════╣
║  POST /api/aegis/events   — <EDR_VENDOR> EDR (Bearer token)      ║
║  POST /api/aegis/posture  — <MDM_VENDOR> 桌管 (Bearer token)    ║
║  POST /hooks/aegis        — Security Webhook (Bearer secret) ║
║  GET  /health             — Health check                    ║
║  GET  /events             — View received events (debug)    ║
╠══════════════════════════════════════════════════════════════╣
║  Tokens (also in .dev/vendor.env):                          ║
║    <EDR_VENDOR_UPPER>_EDR_TOKEN={<EDR_VENDOR_UPPER>_TOKEN[:20]}...  ║
║    <MDM_VENDOR_UPPER>_TOKEN={<MDM_VENDOR_UPPER>_TOKEN[:20]}...     ║
║    AEGIS_WEBHOOK_SECRET={WEBHOOK_SECRET[:16]}...  ║
╚══════════════════════════════════════════════════════════════╝

Adapter config for local testing (aegis-adapters.example.json):
  allowed_hosts: ["127.0.0.1"]
  <edr_vendor>.url:   http://127.0.0.1:{args.port}/api/aegis/events
  <mdm_vendor>.url:  http://127.0.0.1:{args.port}/api/aegis/posture
