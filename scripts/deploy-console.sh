...
# 真实域名不入库/不进 GitHub。
# Mac entry points remain byte-identical to the verified release bundle. Their
# approved package URL/digest/publisher are supplied through deployment config.
for f in aegis-install-windows-oneclick.ps1 install-aegis-windows.cmd; do
