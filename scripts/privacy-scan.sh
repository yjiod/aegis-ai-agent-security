...
scan vendor '(REDACTED_VENDOR|REDACTED_VENDOR|REDACTED_VENDOR|REDACTED_VENDOR|REDACTED_PORTAL|REDACTED_PORTAL|REDACTED_MDM)'
# 2) 真实个人/主机标识（用户名/主机名/.local mDNS）
scan personal '(REDACTED_USER|REDACTED_HOST|REDACTED_HOST|[Mm]acbook-Pro|mac-mini|\.local\b)'
# 3) 真实域名（GitHub 组织 yjiod 允许；真实主机域名禁止）
scan domain '(tx\.yjiod|\.yjiod\.com)'
# 4) 真实工号
scan employee '(REDACTED_EMPID)'
