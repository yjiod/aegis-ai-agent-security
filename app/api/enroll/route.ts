...
import { ensurePolicyReleasesLoaded, currentPolicyRelease, ed25519VerifyKeyInfo } from '@/lib/policy';
...
export async function POST(request: Request) {
  ...
  // 带外信任锚：控制台 ed25519 公钥（公开信息，同 /api/policy/verify-key 源）。
  // 真机修复: 发布存储的 policy 体不带 ed25519 字段(ed 仅在下发工件时附加), 此前从
  // rel.policy 读恒为空 → 安装器从未拿到信任锚。改从密钥派生函数直接取, 与 verify-key 同源。
  const keyInfo = await ed25519VerifyKeyInfo().catch(() => null);
  if (keyInfo) {
    payload.ed25519_public = keyInfo.public;
    payload.ed25519_key_id = keyInfo.key_id;
  }
