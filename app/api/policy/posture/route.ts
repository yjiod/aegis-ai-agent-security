 ...
 * /v1/devices（agent 上报的真实 policy_version，经 lib/collector-devices 共享 30s
 * 缓存拉取）；Collector 不可达时诚实回退控制台注册表，并在响应里用 source/connected
 * 标明来源——绝不把注册表的默认 0.0.0 当成真实终端版本而系统性误报 drifted。
 * 无发布件时返回 published:false。
 */
export async function GET(request: Request) {
  ...
  const versions: Array<string | undefined> =
    collectorDevices !== null
      ? collectorDevices.map((d) => d.policy_version)
