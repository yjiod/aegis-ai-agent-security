...
python3 -c 'import json,os,stat,sys; p=sys.argv[1]; r=json.load(open(p)); inv=r["inventory"]; assert r["schema"]=="sentinel.report/v1" and r["agent_version"]=="0.44.0" and r["policy_version"]=="5.1.0"; assert r["summary"]["critical"]==0 and r["summary"]["high"]==0; assert any(x.get("type")=="ai_agent" and x.get("name")=="codex" for x in inv); assert any(x.get("type")=="agent_config" for x in inv); assert any(x.get("type")=="agent_baseline" and x.get("name")=="codex" and x.get("status")=="managed" for x in inv); assert stat.S_IMODE(os.stat(p).st_mode)==0o600' "$REPORT"
