...
class SentinelTests(unittest.TestCase):
            ...
            with patch.dict(os.environ,env,clear=True):
                ...
                try:
                    ...
                    with urllib.request.urlopen(devices_request,timeout=3) as response:
                        devices=json.load(response); self.assertEqual(devices['devices'][0]['credential_generation'],'current'); self.assertGreaterEqual(devices['generated_at'],now)
    ...
    def test_device_credential_provisioning_is_private_atomic_and_rotation_safe(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            evidence=root/'devices-export.json'; evidence.write_text(json.dumps({'generated_at':1000,'devices':[{'device_id':ids[0],'last_seen':999,'report_count':2,'credential_generation':'previous'}]}))
            ...
            evidence.write_text(json.dumps({'generated_at':1,'devices':[{'device_id':ids[0],'last_seen':1,'report_count':3,'credential_generation':'current'}]}))
