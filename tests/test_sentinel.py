...
class SentinelTests(unittest.TestCase):
    ...
    def test_client_update_policy_keeps_external_lifecycle_owner(self):
        ...
        device={'agent_version':'0.43.0','management_platform':'mdm','dual_slot_ready':True,'consecutive_update_failures':0,'rollout_ring':'pilot'}
        offer={'agent_version':'0.44.0','platform_signature_verified':True,'release_signature_verified':True,'authorized_rings':['lab','pilot']}
    ...
    def test_controlled_self_update_is_fail_closed_and_content_keeps_lkg(self):
        ...
        device={'agent_version':'0.43.0','management_platform':'unmanaged','dual_slot_ready':True,'consecutive_update_failures':0,'rollout_ring':'lab'}
        offer={'agent_version':'0.44.0','platform_signature_verified':True,'release_signature_verified':True,'authorized_rings':['lab']}
    ...
    def test_github_release_gate_uses_native_windows_and_macos_runners(self):
        ...
        for directive in ('Start-Process -FilePath',"'-NoProfile'",'-ManagedUsersRoot','-EncodedCommand','Text.Encoding]::Unicode','RedirectStandardError',"agent_version -cne '0.43.0'","policy_version -cne 'invalid'","policy_load_failed","name -eq 'codex'","status -eq 'managed'",'sentinel-managed-user-baseline:start','hardcoded_secret','$reportText.Contains($secret)','instruction ACL changed','.AGENTS.md.*.tmp','repository managed baseline','repository instruction ACL changed','repository atomic update left'):
    ...
    def test_collector_binds_device_identity_to_independent_credentials(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.dict(os.environ,env,clear=True):
                ...
                try:
                    now=int(time.time()); report={'schema':'sentinel.report/v1','agent_version':'0.43.0','policy_version':'5.1.0','device_id':device_id,'scanned_at':now,'summary':{'critical':0,'high':0,'medium':0,'low':0},'findings':[]}; body=json.dumps(report).encode(); url=f'http://127.0.0.1:{server.server_port}/v1/reports'
                    ...
                    with urllib.request.urlopen(console_request,timeout=3) as response:
                        console=json.load(response)['devices'][0]; self.assertEqual((console['severity'],console['agent_version'],console['policy_version']),('normal','0.43.0','5.1.0'))
                    self.collector.store_report(server.db_path,b'other',{'device_id':'000000000000','agent_version':'0.43.0','policy_version':'5.1.0','summary':{'critical':0,'high':0}},now=now)
    ...
    def test_endpoint_integrity_manifest_covers_all_runtime_inputs(self):
        ...
        self.assertIn("agent_version='0.43.0'",(DOWNLOADS/'sentinel-windows.ps1').read_text())
        self.assertEqual(self.agent.report_headers(b'{}')['User-Agent'],'SentinelAgent/0.43.0')
    ...
    def test_macos_compliance_recomputes_report_summary(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            value={'schema':'sentinel.report/v1','agent_version':'0.43.0','policy_version':self.policy['version'],'device_id':'abcdef123456','scanned_at':now,'summary':{'critical':0,'high':1,'medium':0,'low':0},'findings':[{'kind':'test','severity':'high','path':'x','message':'test'}]}; report.write_text(json.dumps(value))
