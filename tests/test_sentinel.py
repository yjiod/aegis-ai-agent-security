...
class SentinelTests(unittest.TestCase):
    ...
    def test_production_keyring_rotation_requires_retained_key_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            acceptance.write_text(json.dumps(self.production_signer.sign(evidence,keys,'new-key')))
    ...
    def test_console_does_not_claim_live_data_or_fake_task_dispatch(self):
        ...
        for label in ('Gemini CLI','GitHub Copilot CLI','生产就绪清单','生产验收证据模板','生产最终预检','生产验收签名工具','生产验收密钥环工具','Collector 验收探针','Collector API 规范','厂商联动契约','厂商验收证据模板','厂商接入预检','厂商安全验收探针','厂商验收签名工具','Intune 部署清单','Windows 企业签名工具','Intune 晋级证据模板','Intune 晋级预检','Intune 证据生成器','Graph 导出归一化器'): self.assertIn(label,page)
