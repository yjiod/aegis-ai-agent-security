...
class SentinelTests(unittest.TestCase):
    ...
    def test_client_update_policy_keeps_external_lifecycle_owner(self):
        ...
        offer={'agent_version':'0.53.0','platform_signature_verified':True,'release_signature_verified':True,'authorized_rings':['lab','pilot']}
    ...
    def test_controlled_self_update_is_fail_closed_and_content_keeps_lkg(self):
        ...
        offer={'agent_version':'0.53.0','platform_signature_verified':True,'release_signature_verified':True,'authorized_rings':['lab']}
    ...
    def test_unified_service_host_has_fixed_scanner_and_atomic_health_contract(self):
        ...
        for directive in ('UseShellExecute = false','ArgumentList.Add','scanner_shutdown','scanner_timeout','process.Kill(entireProcessTree: true)','File.Move(temp, statusPath, true)','arbitrary_command_enabled = false','--once','--service','--enroll','RunEnrollmentAsync','--status','sentinel.local-status/v1','contains_device_identity = false','contains_secrets = false','ReadBoundedJsonAsync','StartServiceCtrlDispatcher','RegisterServiceCtrlHandlerEx','ServiceControlShutdown','TimeSpan.FromHours(1)'):
    ...
    def test_native_host_is_integrated_into_windows_and_macos_installers(self):
        ...
        for directive in ('sc.exe create','Start-Service','Get-CimInstance Win32_Service','PathName','service-health.json',"host_version -cne '0.5.0'",'Stop-Service','sc.exe delete'):
            self.assertIn(directive,service_smoke)

    def test_service_health_is_strict_minimized_and_fail_closed(self):
        now=int(time.time())
        with tempfile.TemporaryDirectory() as directory:
            path=Path(directory)/'service-health.json'; base={'schema':'sentinel.service-health/v1','host_version':'0.5.0','state':'healthy','service_started_at':now-60,'updated_at':now,'last_scan_started_at':now-10,'last_scan_exit_code':0,'scanner':'python3','error':None,'arbitrary_command_enabled':False}
    ...
    def test_user_session_attestation_is_minimized_and_never_authoritative(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            path=Path(directory)/'session-attestation.json'; value={'schema':'sentinel.user-session/v1','host_version':'0.5.0','updated_at':now,'platform':'macos','agents':['codex','workbuddy'],'baseline_targets':1,'baseline_writes':1,'baseline_failures':0,'arbitrary_command_enabled':False}
    ...
    def test_github_release_gate_uses_native_windows_and_macos_runners(self):
        ...
        for directive in ('Start-Process -FilePath',"'-NoProfile'",'-ManagedUsersRoot','-EncodedCommand','Text.Encoding]::Unicode','RedirectStandardError',"agent_version -cne '0.53.0'","policy_version -cne 'invalid'","policy_load_failed","name -eq 'codex'","status -eq 'managed'",'sentinel-managed-user-baseline:start','hardcoded_secret','$reportText.Contains($secret)','instruction ACL changed','.AGENTS.md.*.tmp','repository managed baseline','repository instruction ACL changed','repository atomic update left'):
    ...
    def test_collector_summary_measures_policy_key_rotation_coverage(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            for index,inventory in enumerate(inventories):
                report={'device_id':f'device-{index}','agent_version':'0.53.0','policy_version':'5.1.0','summary':{'critical':0,'high':0},'inventory':inventory}; self.collector.store_report(db,json.dumps(report).encode(),report,now=now)
    ...
    def test_collector_builds_deterministic_approval_only_recommendations(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            for device,severity,inventory in samples:
                ...
                with self.collector.db_open(path) as db: db.execute("INSERT INTO reports(report_hash,device_id,received_at,severity,body,agent_version,policy_version) VALUES(?,?,?,?,?,?,?)",(device,device+'-device',now,severity,body,'0.53.0','5.1.0')); db.commit()
    ...
    def test_endpoint_integrity_manifest_covers_all_runtime_inputs(self):
        ...
        self.assertIn("agent_version='0.53.0'",(DOWNLOADS/'sentinel-windows.ps1').read_text())
        self.assertEqual(self.agent.report_headers(b'{}')['User-Agent'],'SentinelAgent/0.53.0')
    ...
    def test_macos_compliance_recomputes_report_summary(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            value={'schema':'sentinel.report/v1','agent_version':'0.53.0','policy_version':self.policy['version'],'device_id':'abcdef123456','scanned_at':now,'summary':{'critical':0,'high':1,'medium':0,'low':0},'findings':[{'kind':'test','severity':'high','path':'x','message':'test'}]}; report.write_text(json.dumps(value))
