...
function Invoke-AegisAgent([switch]$Diagnostics) {
  ...
  $messages = @()
  if (Test-Path $stdout) { $messages += Get-Content -Encoding UTF8 $stdout -Raw }
  if (Test-Path $stderr) { $messages += Get-Content -Encoding UTF8 $stderr -Raw }
...
try {
  ...
  $report = Get-Content -Encoding UTF8 $output -Raw | ConvertFrom-Json
  ...
  $reportText = if(Test-Path $output){Get-Content -Encoding UTF8 $output -Raw}else{'[report missing]'}
  ...
  if (-not (Test-Path (Join-Path $codex 'AGENTS.md')) -or (Get-Content -Encoding UTF8 (Join-Path $codex 'AGENTS.md') -Raw) -notmatch 'aegis-managed-user-baseline:start') { throw "Codex managed baseline was not installed; agent=$($result.Output); inventory=$($report.inventory|ConvertTo-Json -Compress -Depth 5)" }
  ...
  if ((Get-Content -Encoding UTF8 $repoInstruction -Raw) -notmatch 'aegis-managed-baseline') { throw 'repository managed baseline was not installed' }
  ...
  $policy = Get-Content -Encoding UTF8 $policyPath -Raw | ConvertFrom-Json
