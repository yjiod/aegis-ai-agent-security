...
try {
  ...
  $ErrorActionPreference = 'Stop'
  if ($agentExitCode -ne 0) { throw "clean agent run failed with exit code $agentExitCode`: $($agentOutput | Out-String)" }
  ...
  $agentExitCode = $LASTEXITCODE
  $ErrorActionPreference = 'Stop'
  if ($agentExitCode -ne 2) { throw "invalid policy did not fail closed; exit code $agentExitCode`: $($agentOutput | Out-String)" }
  $report = Get-Content $output -Raw | ConvertFrom-Json
