#!/usr/bin/env python3
"""Secret-free, non-destructive staging probe for <EDR_VENDOR> and <MDM_VENDOR> adapters."""
import argparse, hashlib, json, os, sys, time
from pathlib import Path
import aegis_adapter as adapter

SCHEMA="aegis.vendor-probe/v1"
VENDORS=("<edr_vendor>","<mdm_vendor>")
...
def run_probe(config,vendor,live=False,sender=adapter.send,now=None):
    ...
    if vendor=="<edr_vendor>": probe_target["actions"]={"normal":"observe"}
    adapter.validate_target(vendor,probe_target,config,dry_run=not live)
    report=synthetic_report(now)
    payload=adapter.<edr_vendor>_event(report,probe_target) if vendor=="<edr_vendor>" else adapter.<mdm_vendor>_posture(report,probe_target,now)
