...
TOP={"schema","release_version","release_manifest_sha256","git_commit_sha","site_version","generated_at","checks","evidence_sha256","approvals","integrity","secrets_embedded","device_identifiers_embedded"}
SIGNING_KEYS_ENV="SENTINEL_PRODUCTION_ACCEPTANCE_SIGNING_KEYS"
...
def evaluate(downloads,evidence,expected_git_commit,expected_site_version,signing_keys,now=None):
    ...
    if evidence.get("schema")!="sentinel.production-acceptance/v2": errors.append("invalid_evidence_schema")
    ...
    if type(approvals) is not dict or set(approvals)!=APPROVALS: errors.append("invalid_approvals_contract")
        ...
        for name in sorted(APPROVALS):
            ...
            if type(value) is not dict or set(value)!={"identity","evidence_sha256"}: errors.append("approval_missing:"+name); continue
