...
def sync_policy(path,report_url,token,verification_keys,timeout=10):
    ...
    endpoint=f"https://{parsed.netloc}/v1/policy"; request=urllib.request.Request(endpoint,headers={"Authorization":"Bearer "+token,"Accept":"application/json","User-Agent":"SentinelAgent/0.52.0"})
...
def report_headers(body,token="",secret="",now=None,device_id=""):
    headers={"Content-Type":"application/json","User-Agent":"SentinelAgent/0.52.0"}
...
def build_report(root,policy,verify_baselines=False,enforce=False,quarantine_root=None,policy_verification_keys=None,device_id=None):
    ...
    return {"schema":"sentinel.report/v1","agent_version":"0.52.0","policy_version":policy["version"],"device_id":device_id,"scanned_at":int(time.time()),"scan_root":safe_path(root),"inventory":inventory,"summary":{s:sum(f["severity"]==s for f in findings) for s in ["critical","high","medium","low"]},"findings":findings}
