...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--evidence",required=True); ap.add_argument("--key-id",required=True); args=ap.parse_args()
    try:
        evidence=preflight.read_json_bounded(args.evidence); signed=sign(evidence,os.getenv(SECRET_ENV,""),args.key_id)
