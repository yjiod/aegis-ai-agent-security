...
适用版本：产品 5.14.0、Endpoint Agent 0.52.0。当前 MSI/PKG 是未完成企业代码签名与 Apple 公证的试点验证包，不得直接晋级生产。
...
```text
msiexec /i Sentinel-Agent-Windows-x64-5.14.0.msi /qn /l*v %ProgramData%\SentinelAgent-install.log
```

macOS root 安装：

```text
installer -pkg Sentinel-Agent-macOS-5.14.0.pkg -target /
```
...
```text
msiexec /x Sentinel-Agent-Windows-x64-5.14.0.msi /qn /l*v %ProgramData%\SentinelAgent-uninstall.log
```
