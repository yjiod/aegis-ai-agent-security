...
only Sentinel's minimized posture event and never performs an access-control
...
def valid_event(value):
    ...
    return (
        value.get("schema") == "sentinel.enterprise-4a.event/v1"
        and value.get("event_type") == "ai_agent_security_posture"
        and value.get("source") == "sentinel"
...
class Receiver(BaseHTTPRequestHandler):
    server_version = "Sentinel4AReceiver/0.1"
    ...
    def authorized(self):
        ...
        expected = os.environ.get("SENTINEL_4A_ACCESS_TOKEN", "")
...
def main():
    ...
    parser.add_argument("--db", default="/var/lib/sentinel/4a-audit.db")
