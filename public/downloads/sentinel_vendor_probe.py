#!/usr/bin/env python3
"""Secret-free, non-destructive staging probe for Sangfor and Leagsoft adapters."""
import argparse, hashlib, json, os, sys, time
from pathlib import Path
import sentinel_adapter as adapter

SCHEMA="sentinel.vendor-probe/v1"
VENDORS=("sangfor","leagsoft")

def synthetic_report(now):
    return {"schema":"sentinel.report/v1","agent_version":"probe","policy_version":"probe","device_id":"sentinel-probe","scanned_at":now,"summary":{"critical":0,"high":0,"medium":0,"low":0},"findings":[]}
...
def run_probe(config,vendor,live=False,sender=adapter.send,now=None):
    ...
    if vendor=="sangfor": probe_target["actions"]={"normal":"observe"}
    adapter.validate_target(vendor,probe_target,config,dry_run=not live)
    report=synthetic_report(now)
    payload=adapter.sangfor_event(report,probe_target) if vendor=="sangfor" else adapter.leagsoft_posture(report,probe_target,now)
