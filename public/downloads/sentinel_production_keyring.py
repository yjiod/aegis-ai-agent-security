...
def verify_retirement_evidence(path,keys,removed_key_id,now=None):
    ...
    if type(evidence) is not dict or set(evidence)!=preflight.TOP or evidence.get("schema")!="sentinel.production-acceptance/v2": raise ValueError("invalid_production_retirement_evidence")
    checks=evidence.get("checks",{}); digests=evidence.get("evidence_sha256",{}); approvals=evidence.get("approvals",{})
    if type(checks) is not dict or set(checks)!=preflight.CHECKS or any(value is not True for value in checks.values()) or type(digests) is not dict or set(digests)!=preflight.CHECKS or any(not isinstance(value,str) or not re.fullmatch(r"[0-9a-f]{64}",value) for value in digests.values()): raise ValueError("invalid_production_retirement_evidence")
    if type(approvals) is not dict or set(approvals)!=preflight.APPROVALS or any(type(value) is not dict or set(value)!={"identity","evidence_sha256"} or not isinstance(value["identity"],str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9 ._@-]{0,127}",value["identity"]) or not isinstance(value["evidence_sha256"],str) or not re.fullmatch(r"[0-9a-f]{64}",value["evidence_sha256"]) for value in approvals.values()): raise ValueError("invalid_production_retirement_evidence")
