...
def verify_retirement_evidence(path,keys,removed_key_id,now=None):
    ...
    if type(evidence) is not dict or set(evidence)!=preflight.TOP or evidence.get("schema")!="sentinel.production-acceptance/v3": raise ValueError("invalid_production_retirement_evidence")
