...
def check_read_only(base,token):
    ...
    required={"generated_at","active_window_seconds","required_agent_version","required_policy_version","total_devices","active_devices","stale_devices","latest_severity","version_posture","credential_posture","agent_coverage","baseline_coverage","service_health_posture","policy_trust_posture","active_policy_key_id"}
...
def check_write(base,token,secret,device_id):
    ...
    now=int(time.time()); report={"schema":"sentinel.report/v1","agent_version":"0.52.0","policy_version":"5.1.0","device_id":device_id,"scanned_at":now,"summary":{"critical":0,"high":0,"medium":0,"low":0},"findings":[]}
