...
密钥环变更必须使用 `sentinel_policy_keyring.py`。新增操作由 CSPRNG 生成密钥、原子保留原所有者/权限且不输出密钥；删除操作必须同时验证最新私有舰队摘要和统一身份/4A 产生的短时、目标绑定审批。只要存在离线设备、Legacy 配置、未知 Key ID、证据或审批过期，或目标是当前/最后一个密钥，删除都会失败关闭；成功后审批证据被消费，避免跨操作重放。
