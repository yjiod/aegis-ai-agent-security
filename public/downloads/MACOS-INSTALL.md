...
原生安装包在替换主程序前确认旧系统 Agent 服务已移除，保留上一主程序并使用原子替换。`native-runtime-activation.json` 的未完成记录会使健康检查失败；服务注册完成仍需独立核实扫描和上报。旧主程序恢复只恢复二进制、不会自动启动服务，也不等于策略、凭据和整个安装包回滚。维护功能在客户端内嵌，不调用用户机器上的 Python。详见仓库 `docs/MACOS-NATIVE-RUNTIME-ACTIVATION.md`。
