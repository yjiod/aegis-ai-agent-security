#!/usr/bin/env python3
"""Normalize privacy-minimized Microsoft Graph Intune snapshots for Sentinel evidence."""
...
SENTINEL_ID=re.compile(r"[A-Za-z0-9._-]{8,128}")
...
def normalize(snapshot):
    ...
    if not isinstance(snapshot,dict) or set(snapshot)!=expected or snapshot.get("schema")!="sentinel.intune-graph-export/v1": raise ValueError("invalid_graph_export")
    ...
    mapping={}; sentinel_ids=set()
    for item in bindings:
        if not isinstance(item,dict) or set(item)!={"intune_device_id","sentinel_device_id"}: raise ValueError("invalid_device_binding")
        graph_id=identifier(item.get("intune_device_id")); sentinel_id=identifier(item.get("sentinel_device_id"),SENTINEL_ID)
        if graph_id in mapping or sentinel_id in sentinel_ids: raise ValueError("invalid_device_binding")
        mapping[graph_id]=sentinel_id; sentinel_ids.add(sentinel_id)
    if set(mapping)!=set(assigned) or not set(assigned).issubset(install): raise ValueError("incomplete_assigned_device_evidence")
    assigned_sentinel=[mapping[value] for value in assigned]
    return {"schema":"sentinel.intune-export/v2","generated_at":generated,"current_ring":snapshot["current_ring"],"fleet_total_devices":len(compliance),"assigned_device_ids":assigned_sentinel,"compliant_device_ids":[mapping[value] for value in assigned if compliance[value]=="compliant"],"installation_failed_device_ids":[mapping[value] for value in assigned if install[value] in FAILURE_STATES]}
