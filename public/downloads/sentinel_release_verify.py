...
BUNDLE_FILES=(
    ...
    "sentinel-rule-sources.json","sentinel_rule_updater.py","sentinel-rule-update.service","sentinel-rule-update.timer","sentinel_policy_keyring.py",
...
def verify(downloads):
    ...
    if release.get("component_versions")!={"endpoint_agent":"0.52.0","policy":"5.1.0","collector":"0.26","adapter":"0.19"}: errors.append("release_component_version_drift")
    ...
    try: user_session_schema=json.loads((downloads/"sentinel-user-session.schema.json").read_text(encoding="utf-8"))
    except (OSError,UnicodeError,ValueError) as exc: errors.append(f"invalid_user_session_schema:{type(exc).__name__}"); user_session_schema={}
    required_user_session={"schema","host_version","updated_at","platform","agents","baseline_targets","baseline_writes","baseline_failures","arbitrary_command_enabled"}
    if user_session_schema.get("additionalProperties") is not False or set(user_session_schema.get("required",[]))!=required_user_session or user_session_schema.get("properties",{}).get("arbitrary_command_enabled")!={"const":False} or user_session_schema.get("properties",{}).get("agents",{}).get("maxItems")!=10: errors.append("unsafe_user_session_schema")
    ...
    for directive in ("DataProtectionScope]::LocalMachine","SENTINEL_REPORT_SIGNING_SECRET","Report token and signing secret must be independent","icacls.exe","sentinel.reporting/v2","PolicyVerificationKeys","policy_verification_keys"):
        if directive not in windows_config: errors.append(f"unsafe_windows_reporting_config:{directive}")
    for directive in ("ProtectedData]::Unprotect","reporting_config_invalid","report_token,report_url,schema,signing_secret","policy_verification_keys,report_token,report_url,schema,signing_secret","X-Sentinel-Policy-Signature","HMACSHA256"):
    ...
    for directive in ("Sentinel AI Agent Security Host","Sentinel AI Agent Security User Bridge","SentinelAIAgentSecurity","Stop-Service","WaitForStatus","sc.exe delete","session-attestation.json","SentinelAgent-Uninstall-Evidence","refusing recursive removal","Quarantine evidence and disabled user objects remain"):
        if directive not in uninstall_windows: errors.append(f"unsafe_windows_uninstaller:{directive}")
    for directive in ("root is required","com.yjiod.sentinel-agent.plist","com.yjiod.sentinel-user-session.plist","session-attestation.json","SentinelAgent-Uninstall-Evidence","refusing recursive removal","Quarantine evidence and disabled user objects remain"):
    ...
    for directive in ("sentinel.device-enrollment/v3","valid_report_url","300<=ttl<=86400","consume_once","expires_at","--policy-keyring","load_policy_keys","policy_verification_keys","policy_key_reused"):
        if directive not in credential_tool: errors.append(f"unsafe_enrollment_generator:{directive}")
    for source,label,directives in ((enroll_windows,"windows",("Administrator is required","ReparsePoint","Enrollment device identity mismatch","Remove-Item -LiteralPath")),(enroll_macos,"macos",("info.st_uid!=0","0o600","enrollment device identity mismatch","source.unlink()"))):
        for directive in directives:
            if directive not in source: errors.append(f"unsafe_{label}_enrollment:{directive}")
    for directive in ("def valid_remediation_receipt","def store_remediation_receipt","invalid_state_transition","approval_callback_token_reused",'self.path=="/v1/remediation-receipts"',"remediation_receipts","actor_ref"):
        if directive not in collector_text: errors.append(f"missing_remediation_receipt_boundary:{directive}")
    ...
    if openapi.get("openapi")!="3.1.0" or openapi.get("info",{}).get("version")!="0.26.0": errors.append("collector_openapi_version_drift")
    ...
    recommendation_schema=components.get("schemas",{}).get("RemediationRecommendation",{}); recommendation_required={"recommendation_id","device_id","reason","recommended_action","approval_state","severity","observed_at","correlation_id","workflow_state","receipt_updated_at"}
    if set(recommendation_schema.get("required",[]))!=recommendation_required or recommendation_schema.get("properties",{}).get("approval_state")!={"const":"external_approval_required"}: errors.append("collector_openapi_recommendation_drift")
    receipt_post=paths.get("/v1/remediation-receipts",{}).get("post",{}); receipt_schema=components.get("schemas",{}).get("RemediationReceipt",{})
    if receipt_post.get("security")!=[{"callbackBearerAuth":[]}] or set(receipt_schema.get("required",[]))!={"schema","recommendation_id","state","external_event_id","actor_id","occurred_at"} or receipt_schema.get("properties",{}).get("actor_id",{}).get("writeOnly") is not True: errors.append("collector_openapi_receipt_drift")
    ...
    for directive in ('parser.add_argument("--write-test",action="store_true")','parsed.scheme!="https"','parsed.username or parsed.password or parsed.query or parsed.fragment','expected=(401,)','/v1/devices?limit=10000','/v1/recommendations','recommendations_contract_invalid','X-Sentinel-Device-ID','digest[:20]','expected_duplicate','hmac.compare_digest(token,secret)','MAX_RESPONSE=1_000_000'):
