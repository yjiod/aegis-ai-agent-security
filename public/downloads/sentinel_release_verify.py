...
def verify(downloads):
    ...
    expected_evidence={"schema":"sentinel.intune-evidence/v2","release_version":release.get("release"),"manifest_sha256":digest(downloads/"intune-deployment-manifest.json"),"generated_at":0,"current_ring":"lab","current_ring_entered_at":0,"collector_probe_read_only_passed":False,"release_verifier_passed":False,"reporting_credentials_delivered_out_of_band":False,"rollback_tested_in_ring":False,"critical_findings":0,"reporting_healthy_since":0,"production_signature_verified":False}
    ...
    if vendor_evidence.get("schema")!="sentinel.vendor-acceptance/v1" or vendor_evidence.get("adapter_version")!="0.18" or vendor_evidence.get("secrets_embedded") is not False: errors.append("unsafe_vendor_acceptance_example")
    for directive in ('max_age_seconds=604800','vendor_endpoint_not_accepted','vendor_gate_failed','vendor_acceptance_must_be_secret_free','auth_scheme','before=path.lstat()','getattr(os,"O_NOFOLLOW",0)','os.fstat(fd)','(opened.st_dev,opened.st_ino)!=(before.st_dev,before.st_ino)','handle.read(max_bytes+1)'):
