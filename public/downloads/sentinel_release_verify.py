...
def verify(downloads):
    ...
    if vendor_contract.get("schema")!="sentinel.vendor-contracts/v1" or vendor_contract.get("adapter_version")!="0.17" or vendor_contract.get("secrets_embedded") is not False: errors.append("vendor_contract_version_drift")
    ...
    if vendor_evidence.get("schema")!="sentinel.vendor-acceptance/v1" or vendor_evidence.get("adapter_version")!="0.17" or vendor_evidence.get("secrets_embedded") is not False: errors.append("unsafe_vendor_acceptance_example")
    ...
    for directive in ('--acceptance','vendor_acceptance_failed','adapter_version="0.17"','now=now','adapter.read_json_bounded','length(CAST(r.body AS BLOB))','adapter.MAX_VENDOR_PAYLOAD_BYTES','hmac.compare_digest','rejected_stored_report_digest_mismatch','rejected_oversized_stored_report'):
