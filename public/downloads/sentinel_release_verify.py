...
def verify(downloads):
    ...
    for directive in ('SentinelAdapter/0.16','MAX_VENDOR_PAYLOAD_BYTES=2_000_000','MAX_VENDOR_QUEUE_FILE_BYTES=2_100_000','MAX_VENDOR_CONFIG_BYTES=65_536','MAX_VENDOR_ACCEPTANCE_BYTES=262_144','def read_json_bounded','path.is_symlink() or not path.is_file()','raw.decode("utf-8")','adapter_payload_too_large','def read_queued_event','path.lstat()','getattr(os,"O_NOFOLLOW",0)','os.fstat(fd)','(opened.st_dev,opened.st_ino)!=(before.st_dev,before.st_ino)','handle.read(MAX_VENDOR_QUEUE_FILE_BYTES+1)','stat.S_ISREG','action not in SAFE_ACTIONS','set(compliance)!={"max_policy_age_hours","critical_allowed"}','not 1<=age<=168','critical!=0','max_age=config["compliance"]["max_policy_age_hours"]*3600','last_scan>now+300','"stale_policy"','isinstance(report["scanned_at"],bool)','invalid_adapter_payload','if len(files)>=keep: raise OSError("adapter_spool_full")','"result":"retained"','spool.is_symlink()','tempfile.mkstemp','os.fchmod(fd,0o600)','os.fsync(handle.fileno())','os.replace(temp,path)','os.fsync(directory_fd)'):
    ...
    if vendor_contract.get("schema")!="sentinel.vendor-contracts/v1" or vendor_contract.get("adapter_version")!="0.16" or vendor_contract.get("secrets_embedded") is not False: errors.append("vendor_contract_version_drift")
    queue_contract=vendor_contract.get("delivery_queue",{})
    if queue_contract!={"overflow_behavior":"retain_source_report_and_retry","silent_eviction_allowed":False,"credentials_stored":False,"write_semantics":"private_fsync_atomic_replace_directory_fsync","read_semantics":"nofollow_regular_file_inode_bound_bounded_read","symlink_directory_allowed":False,"symlink_event_allowed":False,"maximum_queue_file_bytes":2100000}: errors.append("unsafe_vendor_queue_contract")
    ...
    if vendor_evidence.get("schema")!="sentinel.vendor-acceptance/v1" or vendor_evidence.get("adapter_version")!="0.16" or vendor_evidence.get("secrets_embedded") is not False: errors.append("unsafe_vendor_acceptance_example")
    ...
    for directive in ('--acceptance','vendor_acceptance_failed','adapter_version="0.16"','now=now','adapter.read_json_bounded','length(CAST(r.body AS BLOB))','adapter.MAX_VENDOR_PAYLOAD_BYTES','hmac.compare_digest','rejected_stored_report_digest_mismatch','rejected_oversized_stored_report'):
