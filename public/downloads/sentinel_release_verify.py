...
def verify(downloads):
    ...
    try: production_preflight=(downloads/"sentinel_production_preflight.py").read_text(encoding="utf-8"); production_signer=(downloads/"sentinel_production_evidence_sign.py").read_text(encoding="utf-8"); production_keyring=(downloads/"sentinel_production_keyring.py").read_text(encoding="utf-8"); production_example=json.loads((downloads/"production-acceptance-evidence.example.json").read_text(encoding="utf-8"))
    except (OSError,UnicodeError,ValueError) as exc: errors.append(f"invalid_production_acceptance:{type(exc).__name__}"); production_preflight=production_signer=production_keyring=""; production_example={}
    for directive in ("sentinel.production-acceptance/v2","MAX_EVIDENCE_BYTES=64*1024","MAX_SIGNING_KEYS_BYTES=64*1024","O_NOFOLLOW","unsafe_production_signing_keyring","hmac.compare_digest","SENTINEL_PRODUCTION_ACCEPTANCE_SIGNING_KEYS","evidence_digest_missing:","approval_digest_missing:","invalid_production_signature","--expected-git-commit","--expected-site-version","--keyring","git_commit_mismatch","site_version_mismatch","release_manifest_mismatch","gate_failed:","approval_missing:","device_identifiers_must_not_be_embedded"):
        if directive not in production_preflight: errors.append(f"unsafe_production_preflight:{directive}")
    for directive in ("private_atomic_output","canonical_unsigned","production_signing_key_unavailable","secrets_embedded","--keyring"):
