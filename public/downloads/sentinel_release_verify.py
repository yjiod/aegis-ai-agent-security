...
def verify(downloads):
    ...
    for directive in ('SentinelAdapter/0.11','action not in SAFE_ACTIONS','set(compliance)!={"max_policy_age_hours","critical_allowed"}','not 1<=age<=168','critical!=0','max_age=config["compliance"]["max_policy_age_hours"]*3600','last_scan>now+300','"stale_policy"','isinstance(report["scanned_at"],bool)','invalid_adapter_payload','if len(files)>=keep: raise OSError("adapter_spool_full")','"result":"retained"'):
    ...
    if vendor_contract.get("schema")!="sentinel.vendor-contracts/v1" or vendor_contract.get("adapter_version")!="0.11" or vendor_contract.get("secrets_embedded") is not False: errors.append("vendor_contract_version_drift")
    queue_contract=vendor_contract.get("delivery_queue",{})
    if queue_contract!={"overflow_behavior":"retain_source_report_and_retry","silent_eviction_allowed":False,"credentials_stored":False}: errors.append("unsafe_vendor_queue_contract")
    ...
    if vendor_evidence.get("schema")!="sentinel.vendor-acceptance/v1" or vendor_evidence.get("adapter_version")!="0.11" or vendor_evidence.get("secrets_embedded") is not False: errors.append("unsafe_vendor_acceptance_example")
    ...
    for directive in ('--acceptance','vendor_acceptance_failed','adapter_version="0.11"','now=now'):
