...
BUNDLE_FILES=(
    ...
    "sentinel-adapters.example.json","sentinel_release_verify.py","sentinel_collector_backup.py","sentinel_collector_restore.py","sentinel_vendor_probe.py","sentinel_vendor_evidence_sign.py","sentinel_vendor_keyring.py","sentinel_production_preflight.py","sentinel_production_evidence_sign.py","production-acceptance-evidence.example.json",
...
def verify(downloads):
    ...
    if release.get("component_versions")!={"endpoint_agent":"0.40.0","policy":"4.9.0","collector":"0.19","adapter":"0.18"}: errors.append("release_component_version_drift")
    ...
    try: production_preflight=(downloads/"sentinel_production_preflight.py").read_text(encoding="utf-8"); production_signer=(downloads/"sentinel_production_evidence_sign.py").read_text(encoding="utf-8"); production_example=json.loads((downloads/"production-acceptance-evidence.example.json").read_text(encoding="utf-8"))
    except (OSError,UnicodeError,ValueError) as exc: errors.append(f"invalid_production_acceptance:{type(exc).__name__}"); production_preflight=production_signer=""; production_example={}
    for directive in ("sentinel.production-acceptance/v2","MAX_EVIDENCE_BYTES=64*1024","O_NOFOLLOW","hmac.compare_digest","SENTINEL_PRODUCTION_ACCEPTANCE_SIGNING_KEYS","evidence_digest_missing:","approval_digest_missing:","invalid_production_signature","--expected-git-commit","--expected-site-version","git_commit_mismatch","site_version_mismatch","release_manifest_mismatch","gate_failed:","approval_missing:","device_identifiers_must_not_be_embedded"):
        if directive not in production_preflight: errors.append(f"unsafe_production_preflight:{directive}")
    for directive in ("private_atomic_output","canonical_unsigned","production_signing_key_unavailable","secrets_embedded"):
        if directive not in production_signer: errors.append(f"unsafe_production_signer:{directive}")
    expected_production_checks={"release_verified","ci_gates_passed","collector_probe_passed","collector_backup_restore_tested","intune_production_preflight_passed","windows_upgrade_rollback_tested","macos_upgrade_rollback_tested","sangfor_acceptance_v3_passed","leagsoft_acceptance_v3_passed","console_read_only_connected"}; expected_production_approvals={"security_owner","endpoint_owner","platform_owner","business_owner"}
    integrity=production_example.get("integrity",{}); approvals=production_example.get("approvals",{})
    if production_example.get("schema")!="sentinel.production-acceptance/v2" or production_example.get("release_version")!=release.get("release") or production_example.get("secrets_embedded") is not False or production_example.get("device_identifiers_embedded") is not False or set(production_example.get("checks",{}))!=expected_production_checks or any(production_example.get("checks",{}).values()) or set(production_example.get("evidence_sha256",{}))!=expected_production_checks or set(production_example.get("evidence_sha256",{}).values())!={"REPLACE_WITH_SHA256"} or set(approvals)!=expected_production_approvals or any(not isinstance(value,dict) or set(value)!={"identity","evidence_sha256"} or value!={"identity":"","evidence_sha256":"REPLACE_WITH_SHA256"} for value in approvals.values()) or integrity!={"algorithm":"hmac-sha256","key_id":"REPLACE_WITH_KEY_ID","signature":"REPLACE_WITH_HMAC_SHA256"}: errors.append("unsafe_production_acceptance_example")
    ...
    if vendor_contract.get("schema")!="sentinel.vendor-contracts/v1" or vendor_contract.get("adapter_version")!="0.18" or vendor_contract.get("secrets_embedded") is not False: errors.append("vendor_contract_version_drift")
    ...
    for directive in ('--acceptance','vendor_acceptance_failed','adapter_version="0.18"','now=now','adapter.read_json_bounded','length(CAST(r.body AS BLOB))','adapter.MAX_VENDOR_PAYLOAD_BYTES','hmac.compare_digest','rejected_stored_report_digest_mismatch','rejected_oversized_stored_report'):
