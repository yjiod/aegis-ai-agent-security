...
def verify(downloads):
    ...
    if release.get("component_versions")!={"endpoint_agent":"0.37.0","policy":"4.9.0","collector":"0.19","adapter":"0.18"}: errors.append("release_component_version_drift")
    ...
    for directive in ('"gemini_cli"','"github_copilot_cli"','".gemini/settings.json"','".copilot/mcp-config.json"','".gemini/GEMINI.md"','".copilot/copilot-instructions.md"','cfg.get("httpUrl"','".gemini/skills"','".copilot/skills"','def verify_user_baselines','"agent_baseline_not_loaded"','"type":"agent_baseline"','SentinelAgent/0.37.0','not path.is_symlink()','if root.is_symlink(): return []','not marker.is_symlink()','def atomic_managed_write','os.fchown','os.fsync(handle.fileno())','os.replace(temp,path)'):
        if directive not in python_agent: errors.append(f"missing_python_agent_coverage:{directive}")
    for directive in ("gemini_cli=@(","github_copilot_cli=@(",".gemini\\GEMINI.md","copilot-instructions.md","$cfg.httpUrl","'.gemini','.copilot'","Get-SentinelUserBaselineStatus","type='agent_baseline'","agent_baseline_not_loaded","agent_version='0.37.0'","foreach ($userHome in $userHomes)","function Get-SentinelUserBaselineStatus([string]$userHomePath","[string]$ManagedUsersRoot = 'C:\\Users'","Get-ChildItem $ManagedUsersRoot","Attributes -band [IO.FileAttributes]::ReparsePoint","foreach($secretPattern in @($policy.secret_patterns))","[regex]::new([string]$secretPattern","[TimeSpan]::FromMilliseconds(250)","RegexMatchTimeoutException","scan_rule_timeout","Kind='credential_access'","Kind='dynamic_eval'"):
