...
def verify(downloads):
    ...
    if release.get("component_versions")!={"endpoint_agent":"0.39.0","policy":"4.9.0","collector":"0.19","adapter":"0.18"}: errors.append("release_component_version_drift")
    ...
    for directive in ('function Test-SentinelUserTarget','function Set-SentinelUserTextAtomic','$stream.Flush($true)','Set-Acl -Path $temp -AclObject $acl','Move-Item -LiteralPath $temp -Destination $target -Force','Remove-Item -LiteralPath $temp -Force','Attributes -band [IO.FileAttributes]::ReparsePoint'):
