...
def verify(downloads):
    ...
    for directive in ('SENTINEL_VENDOR_ACCEPTANCE_SIGNING_SECRET','--keyring','load_signing_secret','preflight.read_json_bounded(keyring_path,preflight.MAX_SIGNING_KEYS_BYTES,private=True)','vendor_acceptance_signing_key_unavailable','invalid_unsigned_vendor_acceptance','invalid_vendor_acceptance_signing_secret','hmac.new(secret.encode()','preflight.canonical_unsigned(signed)','"integrity"'):
