...
def verify(downloads):
    ...
    for directive in ('script_signature_state")!="production_signed"','evidence.get("production_signature_verified") is not True','artifact_digest_mismatch','now-generated>86400','now-healthy<86400','RINGS.index(target_ring)!=RINGS.index(current)+1','now-entered<minimum*3600','MAX_INTUNE_EVIDENCE_BYTES=65_536','MAX_INTUNE_MANIFEST_BYTES=262_144','MAX_INTUNE_ARTIFACT_BYTES=2_000_000','def read_bytes_bounded','before=path.lstat()','getattr(os,"O_NOFOLLOW",0)','os.fstat(fd)','(opened.st_dev,opened.st_ino)!=(before.st_dev,before.st_ino)','handle.read(max_bytes+1)','Path(name).name!=name','set(item)!={"file","sha256"}'):
