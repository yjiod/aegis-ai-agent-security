...
管理员交互安装与 MDM 使用完全相同的安装器内容，发布流程生成并校验 `aegis-install-macos-oneclick.sh`，禁止部署时改写 Mac 脚本。公开参数、受保护环境配置和迁移说明见随包提供的 `MACOS-INSTALL.md`。交互入口不再接受 `-Server` 或自动重新下载脚本提权；必须运行经过审核的本地脚本，并提供已批准包摘要及发布者。
...
Mac MDM 安装完整原生 pkg；`rollback-aegis-macos.sh` 现按批准目标包重新部署，要求当前程序摘要或明确缺失状态、目标版本、批准包摘要和发布者，详见随包 MACOS-INSTALL.md。旧三文件复制已移除。包验证失败时不调用 Installer；Installer 执行后失败仍可能部分修改系统，不能当作自动事务回滚。目标可以为早期兼容版本或同版本修复，恢复后必须验收实际版本、策略和上报。完整包事务回滚、真实旧用户服务迁移和签名恢复验收仍为发布门槛，不得重新安装 Python 脚本绕过。
