...
- **就绪预热（ExecStartPost）**：unit 加 `ExecStartPost=/bin/sh -c 'for i in $(seq 1 60); do code=$(curl -s -o /dev/null -w %{http_code} -m 5 http://127.0.0.1:8787/login); if [ "$code" = 200 ] || [ "$code" = 307 ]; then exit 0; fi; sleep 5; done; exit 0'`，使 `systemctl restart` 阻塞到 worker 真正可服务（超时仍 exit 0 不把服务判失败，真挂死交给看门狗）。部署脚本/人工重启后无需再手动轮询就绪。
