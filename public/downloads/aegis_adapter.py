#!/usr/bin/env python3
"""Fail-safe vendor boundary for <EDR_VENDOR> EDR and <MDM_VENDOR>. Disabled by default."""
...
ADAPTERS=("<edr_vendor>","<mdm_vendor>","security_webhook")
TARGET_FIELDS={"<edr_vendor>":{"enabled","mode","url","token_env","actions"},"<mdm_vendor>":{"enabled","mode","url","token_env","compliance"},"security_webhook":{"enabled","mode","url","secret_env"}}
CREDENTIAL_PREFIX={"<edr_vendor>":"<EDR_VENDOR_UPPER>_","<mdm_vendor>":"<MDM_VENDOR_UPPER>_","security_webhook":"AEGIS_"}
...
def validate_config(config):
    ...
    for name in ADAPTERS:
        ...
        if name=="<edr_vendor>":
            ...
            if not isinstance(actions,dict) or not set(actions).issubset({"critical","high","medium","low","normal"}) or any(not isinstance(action,str) for action in actions.values()): raise ValueError("invalid_<edr_vendor>_actions")
    return config
...
def valid_report(report):
    ...
    if not required.issubset(report) or not set(report).issubset(allowed) or report.get("schema")!="aegis.report/v1": return False
...
def valid_payload(name,payload):
    ...
    fields={"<edr_vendor>":{"event_type","source","device_id","severity","recommended_action","finding_count","policy_version","occurred_at"},"<mdm_vendor>":{"source","device_id","compliant","risk_level","policy_version","last_scan","reason"}}
...
def severity(report):
    ...
    return "normal"
def <edr_vendor>_event(report,config):
    level=severity(report); action=config.get("actions",{}).get(level,"observe")
    if action not in SAFE_ACTIONS: raise ValueError(f"unsafe_<edr_vendor>_action:{action}")
...
def <mdm_vendor>_posture(report,config):
...
def send(url,payload,token="",secret=""):
    body=json.dumps(payload,ensure_ascii=False,separators=(",",":")).encode(); headers={"Content-Type":"application/json","User-Agent":"AegisAdapter/0.7","Idempotency-Key":hashlib.sha256(body).hexdigest()}
    if token: headers["Authorization"]="Bearer "+token
    if secret:
        timestamp=str(int(time.time())); headers["X-Aegis-Signature"]="sha256="+hmac.new(secret.encode(),timestamp.encode()+b"."+body,hashlib.sha256).hexdigest(); headers["X-Aegis-Timestamp"]=timestamp
...
def spool_limit(value=None):
    raw=os.getenv("AEGIS_ADAPTER_SPOOL_MAX_EVENTS","500") if value is None else value
...
def process(report,config,dry_run=False,spool_dir=None,sender=send):
    ...
    builders={"<edr_vendor>":<edr_vendor>_event,"<mdm_vendor>":<mdm_vendor>_posture,"security_webhook":lambda value,_config:value}
    for name in ADAPTERS:
        target=config.get(name,{})
    ...
    return outputs
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("report",nargs="?"); ap.add_argument("--config",default=str(Path(__file__).with_name("aegis-adapters.json"))); ap.add_argument("--dry-run",action="store_true"); ap.add_argument("--spool-dir",default=os.getenv("AEGIS_ADAPTER_SPOOL","")); ap.add_argument("--flush-only",action="store_true"); args=ap.parse_args()
    ...
    spool=Path(args.spool_dir) if args.spool_dir else Path.home()/".aegis-adapter/spool"
