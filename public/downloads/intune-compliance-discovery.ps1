...
$expected = @{ 'sentinel-policy.json'='4ebac2abbe3654d048a8af17cf46df3161de336c00d2f5c4c692d90ffeffb832'; 'sentinel-windows.ps1'='e7866bbb1143dab02d44cf2538c7e7ae0301679e02c4a198047b20c20583f447'; 'sentinel-security-baseline.md'='e6d87dba8756aa270a70f423368bf68a44f108a5a299ab2a62c4488ed74a962e' }
...
if(Test-Path $reportingPath){
  try{
    ...
    $keys=@($reporting.policy_verification_keys|ForEach-Object{[string]$_});$contract=(($names -join ',') -ceq 'policy_verification_keys,report_token,report_url,schema,signing_secret' -and $reporting.schema -ceq 'sentinel.reporting/v2') -or (($names -join ',') -ceq 'device_id,policy_verification_keys,report_token,report_url,schema,signing_secret' -and $reporting.schema -ceq 'sentinel.reporting/v3' -and [string]$reporting.device_id -cmatch '^[0-9a-f]{12}$');$reportingConfigured=$contract -and $urlValid -and ([string]$reporting.report_token).Length -ge 32 -and ([string]$reporting.report_token).Length -le 4096 -and ([string]$reporting.signing_secret).Length -ge 32 -and ([string]$reporting.signing_secret).Length -le 4096 -and $reporting.report_token -cne $reporting.signing_secret -and $keys.Count -ge 1 -and $keys.Count -le 5 -and @($keys|Where-Object{$_.Length -lt 32 -or $_.Length -gt 4096}).Count -eq 0 -and @($keys|Select-Object -Unique).Count -eq $keys.Count -and $reporting.report_token -notin $keys -and $reporting.signing_secret -notin $keys
