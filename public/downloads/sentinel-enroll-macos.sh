...
info=source.lstat()
if not stat.S_ISREG(info.st_mode) or info.st_uid!=0 or stat.S_IMODE(info.st_mode)!=0o600 or info.st_size>16384: raise SystemExit("unsafe enrollment file ownership, mode, type, or size")
value=json.loads(source.read_text(encoding="utf-8")); expected={"schema","device_id","report_url","report_token","signing_secret","issued_at","expires_at","consume_once"}
if not isinstance(value,dict) or set(value)!=expected or value.get("schema")!="sentinel.device-enrollment/v2" or value.get("consume_once") is not True: raise SystemExit("invalid enrollment contract")
now=int(time.time())
...
url=value.get("report_url",""); parsed=urlsplit(url); token=value.get("report_token",""); secret=value.get("signing_secret","")
...
try:
    with os.fdopen(fd,"w",encoding="utf-8") as handle:
        json.dump({"schema":"sentinel.reporting/v1","report_url":url,"report_token":token,"signing_secret":secret},handle,separators=(",",":")); handle.flush(); os.fsync(handle.fileno())
