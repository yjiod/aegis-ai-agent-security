...
import sentinel_intune_preflight as preflight
...
def build(downloads,base,intune,collector,now=None):
    ...
    if not isinstance(intune,dict) or set(intune)!={"schema","generated_at","current_ring","fleet_device_ids","assigned_device_ids","compliant_device_ids","installation_failed_device_ids"} or intune.get("schema")!="sentinel.intune-export/v1": raise ValueError("invalid_intune_export")
    ...
    fleet=ids(intune["fleet_device_ids"]); assigned=ids(intune["assigned_device_ids"]); compliant=ids(intune["compliant_device_ids"]); failed=ids(intune["installation_failed_device_ids"])
    if not assigned.issubset(fleet) or not compliant.issubset(assigned) or not failed.issubset(assigned) or compliant&failed: raise ValueError("invalid_intune_device_relationship")
    reporting=set()
    for item in collector["devices"]:
        ...
        device_id=item.get("device_id"); last_seen=item.get("last_seen")
        if not isinstance(device_id,str) or not DEVICE_ID.fullmatch(device_id) or isinstance(last_seen,bool) or not isinstance(last_seen,int) or last_seen>now+60: raise ValueError("invalid_collector_device")
    ...
    result=dict(base); result.update(schema="sentinel.intune-evidence/v3",release_version=release["release"],manifest_sha256=hashlib.sha256(manifest).hexdigest(),generated_at=now,current_ring=intune["current_ring"],fleet_total_devices=len(fleet),ring_assigned_devices=len(assigned),reporting_devices=len(reporting),compliant_devices=len(compliant),installation_failures=len(failed))
    return result
