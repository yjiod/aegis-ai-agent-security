...
if($item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -or $item.Length -gt 32768){throw 'Unsafe enrollment file type or size'}
...
$expected=@('schema','device_id','report_url','report_token','signing_secret','policy_verification_keys','issued_at','expires_at','consume_once')
$names=@($enrollment.PSObject.Properties.Name)
if(@($names|Where-Object{$_ -notin $expected}).Count -or @($expected|Where-Object{$_ -notin $names}).Count -or $enrollment.schema -cne 'sentinel.device-enrollment/v3' -or $enrollment.consume_once -ne $true){throw 'Invalid enrollment contract'}
