...
def send(url,payload,token="",secret=""):
    body=json.dumps(payload,ensure_ascii=False,separators=(",",":")).encode(); headers={"Content-Type":"application/json","User-Agent":"SentinelAdapter/0.3"}
...
def queue_delivery(spool,name,payload):
    ...
    path=spool/f"{int(time.time())}-{digest}.json"; path.write_text(body); os.chmod(path,0o600); return path
def flush_spool(config,spool,sender=send,limit=50):
    ...
    for path in sorted(spool.glob("*.json"))[:limit]:
        try:
            ...
            if name not in ADAPTERS: raise ValueError("unknown_queued_adapter")
        ...
        except Exception as exc: results.append({"adapter":path.name,"result":"retained","error":type(exc).__name__})
    return results
