...
try:
    with os.fdopen(fd,"w",encoding="utf-8") as handle: json.dump({"schema":"sentinel.reporting/v1","report_url":url,"report_token":token,"signing_secret":secret},handle,separators=(",",":")); handle.flush(); os.fsync(handle.fileno())
