...
失败回退：停止导入，不修改终端管理平台、4A、可选厂商适配器或 Collector；重新获取与 Git 提交绑定的完整包。
...
未启用深信服或联软时，不要求对应现网凭据或验收；必须提供“目标关闭、配置中无相关凭据引用”的审计记录。启用任一目标时，才执行本节对应的 v3 厂商验收。
...
| 选定部署平台 lab/pilot/broad/production | 平台晋级证据；Intune 可用 v3 预检输出 |  | 未完成 |
| Windows/macOS 真实终端升级回滚 | MDM 执行记录与合规结果 |  | 未完成 |
| 企业 4A 标准接口 | OpenAPI 映射、幂等、审批和审计证据 |  | 未完成 |
| 可选适配器 | 全部关闭的配置审计，或已启用目标的 v3 验收证据 |  | 未完成 |
| 生产控制台 Collector 连通 | 只读摘要与无秘密浏览器检查 |  | 未完成 |

最终批准：安全负责人、终端管理负责人、平台运维负责人和业务代表均签署后，才能把发行标记为生产可用。

将上述实测结果填写到 `production-acceptance-evidence.example.json` 的副本。每个 `checks` 项必须绑定对应外部验收记录的 SHA-256；每个审批项必须填写审批人身份，并绑定审批记录 SHA-256。替换发行清单 SHA-256、Git 提交、Sites 版本和时间戳后，在受保护的签名工作站运行 `python3 sentinel_production_evidence_sign.py --evidence <已审查未签名证据.json> --evidence-root <原始证据目录> --expected-git-commit <从GitHub独立核验的完整SHA> --expected-site-version <从Sites独立核验的版本> --key-id <当前密钥ID> --keyring <受保护密钥环.json> --output <已签名证据.json>`。签名工具会重新核验当前发行、权威 Git/Sites 值、全部原始记录和门禁；密钥不得写入模板、命令参数、进程环境、日志或工单。
...
把十项检查记录和四方审批记录放入专用目录，按模板填写十四个互不重复的单层文件名、真实检查结果和审批身份。运行 `python3 sentinel_production_evidence_prepare.py --evidence <工作副本.json> --evidence-root <原始证据目录> --output <待签名证据.json>` 自动计算摘要；准备器不会改变检查结论或审批人。人工复核后再签名。最后运行 `python3 sentinel_production_preflight.py <已签名证据.json> --evidence-root <原始证据目录> --keyring <受保护密钥环.json> --expected-git-commit <从 GitHub 核验的完整 SHA> --expected-site-version <从 Sites 核验的版本号>`。预期值必须来自两个系统的独立只读查询而不是证据文件本身；只有输出 `{"ok":true,"errors":[]}` 才构成最终机器门禁。模板默认全部为 false、摘要为空且没有有效签名，不能直接通过。密钥轮换时可短期并存最多五个 key ID；删除旧键必须运行 `sentinel_production_keyring.py --remove-key-id <旧ID> --acceptance <新键签署且24小时内的证据>`，不能删除证据签名所用键或最后一把键。
