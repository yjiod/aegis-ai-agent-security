#!/usr/bin/env python3
"""Build a Sentinel release deterministically from the checked-in sources."""
...
GENERATED_FILES={"CHECKSUMS.sha256","RELEASE-MANIFEST.sha256","intune-deployment-manifest.json","intune-rollout-evidence.example.json","production-acceptance-evidence.example.json","sentinel-enterprise-bundle.zip"}
...
def build(downloads):
    ...
    for name in set(verifier.BUNDLE_FILES)|{"sentinel-enterprise-bundle.zip"}: require_regular(downloads/name)
...
def check(downloads):
    downloads=Path(downloads)
    with tempfile.TemporaryDirectory(prefix="sentinel-release-check-") as directory:
