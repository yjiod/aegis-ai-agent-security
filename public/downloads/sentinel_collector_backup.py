#!/usr/bin/env python3
"""Create and verify an atomic online backup of the Sentinel collector database."""
...
def backup_database(source,directory,keep=14,now=None):
    ...
    final=directory/f"sentinel-backup-{stamp}-{uuid.uuid4().hex[:8]}.sqlite"
    fd,temp_name=tempfile.mkstemp(prefix=".sentinel-backup-",suffix=".tmp",dir=directory); os.close(fd); temp=Path(temp_name)
    try:
    ...
    backups=sorted(directory.glob("sentinel-backup-*.sqlite"),key=lambda item:item.stat().st_mtime,reverse=True)
    for old in backups[keep_count(keep):]:
        if old.is_file() and not old.is_symlink(): old.unlink()
    return final

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--db",default="sentinel.db"); ap.add_argument("--output",required=True); ap.add_argument("--keep",default=14,type=int); args=ap.parse_args()
