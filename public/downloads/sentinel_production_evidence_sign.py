...
def validate_unsigned(evidence,evidence_root,now=None):
...
def sign(evidence,keys,key_id,evidence_root,now=None):
    validate_unsigned(evidence,evidence_root,now=now)
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument("--evidence",required=True);parser.add_argument("--evidence-root",required=True);parser.add_argument("--key-id",required=True);parser.add_argument("--output",required=True);parser.add_argument("--keyring");args=parser.parse_args()
    try:
        evidence=json.loads(preflight.read_regular_bounded(args.evidence,preflight.MAX_EVIDENCE_BYTES));keys=preflight.load_signing_keys(args.keyring) if args.keyring else preflight.parse_signing_keys(os.getenv(preflight.SIGNING_KEYS_ENV,""));result=sign(evidence,keys,args.key_id,args.evidence_root);private_atomic_output(args.output,result)
