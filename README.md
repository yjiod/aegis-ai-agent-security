...
当前发行：产品 `5.15.0`，Endpoint Agent `0.53.0`，策略 `5.1.0`，Collector `0.26`，Adapter `0.19`。

5.15 起，统一宿主提供隐私最小化的 `--status` 运维入口。它只输出平台、宿主版本、服务健康、最近扫描退出码、报告新鲜度和注册配置存在状态，不输出设备 ID、文件路径、发现内容或秘密；输入文件均执行防链接和大小边界检查，非健康状态返回退出码 2，便于任意 MDM/桌管统一检测。

5.14 起，统一原生宿主提供固定 `--enroll <file>` 注册入口。MDM、桌管或 4A 支撑流程在 Windows/macOS 上都只需调用同一个客户端可执行文件；宿主拒绝未知参数、链接和超大注册文件，并对固定随包注册引擎实施两分钟超时与进程树终止。平台脚本保留为包内兼容引擎，不再要求管理员把 PowerShell 当作客户端入口。

5.13 起，设备注册分配的 12 位不透明 `device_id` 持久化到 Windows DPAPI 或 macOS root-only reporting v3，并统一绑定报告正文、HMAC 请求头和离线队列。主机重命名不会改变已注册身份；旧 reporting v1/v2 仍可读取，但会产生高风险迁移项，不能覆盖已绑定身份。

5.12 起，同一个原生宿主新增固定 `--user-session` 会话桥。Windows 由受限 Users 组登录任务触发，macOS 由全局 LaunchAgent 在登录及每小时触发；它只发现声明的 AI Agent 并以当前用户权限原子维护 Codex、Claude Code、Gemini CLI 和 Copilot CLI 的基线受管块。会话证明不含用户名、路径、凭据或 Prompt，系统扫描器仍独立复核基线，不能用用户态证明自行放行。
...
5.6 起，企业 4A 或终端平台可通过独立回调凭据提交审批和执行回执。Collector 强制 pending → approved/rejected → executing → succeeded/failed 状态机，拒绝跳步、过期、未知建议和幂等冲突；操作者身份仅保存不可逆摘要，控制台只展示状态与既有建议关联号，不接收或执行远程命令。

5.6 安装包通过 `deploy/clients/build-installers.sh` 从同一服务宿主源码构建 Windows x64 MSI 与 macOS arm64 PKG。构建后会解包扫描并拒绝任何内嵌上报令牌、签名密钥或历史占位凭据；生产凭据只能由 MDM、桌管或 4A 支撑流程带外配置。此前本地 4.x/5.0/5.1 验证包已撤销，相关生产上报凭据已轮换。
...
5.7 起，MSI/PKG 保持零凭据；企业分发平台另行投递设备绑定、限时、成功后销毁的 `sentinel.device-enrollment/v2` 注册文件。Windows 消费器先验证管理员、设备 ID、期限、固定字段、文件类型与宽 ACL，再通过本机 DPAPI 落盘；macOS 消费器要求 root 所有、0600、非链接普通文件并原子写入 root-only 配置。注册失败保留原文件以便受控排障，成功后立即删除。
