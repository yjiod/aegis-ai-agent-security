...
Sentinel 是面向企业终端的 AI Coding 安全治理工具。它通过 Microsoft Intune 部署，在 Windows、macOS/Linux 上自动发现 Cursor、Claude Code、Codex 和 Windsurf，加载企业安全编码基线，并扫描 Skill、MCP、代码质量与依赖风险。报告可进入受认证的接收器，并通过安全适配边界与<EDR_VENDOR_CN> EDR、<MDM_VENDOR_CN>桌管协同。

当前发行：产品 `0.58.0`，Endpoint Agent `0.30.0`，策略 `4.8.0`，Collector `0.12`，Adapter `0.7`。
...
真实 Intune、<EDR_VENDOR_CN> EDR、<MDM_VENDOR_CN>及 Collector 凭据不得提交到 Git。
